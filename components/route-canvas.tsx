...
export function RouteCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      context.strokeStyle = depthEnabled ? "rgba(41,59,120,.34)" : "rgba(41,59,120,.22)";
      context.lineWidth = depthEnabled ? 1.35 : 1;
      context.shadowColor = depthEnabled ? "rgba(41,59,120,.26)" : "transparent";
      context.shadowBlur = depthEnabled ? 13 : 0;
      ...
      points.forEach(([x, y], index) => {
        ...
        context.fill();
        context.beginPath();
        context.arc(x * width, y * height, depthEnabled ? 15 : 11, 0, Math.PI * 2);
        context.strokeStyle = depthEnabled ? "rgba(41,59,120,.3)" : "rgba(41,59,120,.24)";
        context.stroke();
        if (depthEnabled) {
          context.beginPath();
          context.arc(x * width, y * height, 24, 0, Math.PI * 2);
          context.strokeStyle = "rgba(41,59,120,.09)";
          context.stroke();
        }
      });
    };
